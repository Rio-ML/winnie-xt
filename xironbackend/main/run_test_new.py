...
from xironbackend.data.dependent_data_json import DependentDataJson
from xironbackend.data.Dict import Dict
from xironbackend.data.get_data_new import GetDataNew
...
class RunTest(unittest.TestCase):
    ...
    # 程序执行的
    def go_on_run(self):
        data_dict = self.opera_excel.get_valid_sheets_data()
        for sheet_name in data_dict.keys():
            self.data = GetDataNew(sheet_name=sheet_name)
            ...
            for i in range(2, self.excel_prop.rows_count):
                ...
                if self.excel_prop.is_run:
                    ...
                    if self.excel_prop.case_depend is not None and CommonUtil.is_json(self.excel_prop.case_depend):
                        self.depend_data_json = DependentDataJson(self.excel_prop, self.data)
